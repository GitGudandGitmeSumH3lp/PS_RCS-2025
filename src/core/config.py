...
class Settings:
    ...
    Attributes:
        ...
        LIDAR_ANGLE_OFFSET_DEG: Mounting offset in degrees.
    """
    MOTOR_PORT: str
    LIDAR_PORT: str
    ...
    def load_from_file(cls, filepath: str = "config/settings.json") -> "Settings":
        ...
        # New offset validation
        offset = config_data.get("LIDAR_ANGLE_OFFSET_DEG", 0.0)
        if not isinstance(offset, (int, float)):
            raise ValueError(f"LIDAR_ANGLE_OFFSET_DEG must be a number, got {type(offset).__name__}")
        if not (-180.0 <= float(offset) <= 180.0):
            raise ValueError(f"LIDAR_ANGLE_OFFSET_DEG must be in [-180, 180], got {offset}")
        ...
        return cls(
            ...
            LIDAR_ANGLE_OFFSET_DEG=float(offset)
        )
